...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Medication selection for reorder section - FINAL FIX
  const medicationOptions = document.querySelectorAll('.medication-option');
  
  // Ensure only one is active on page load
  if (medicationOptions.length > 0) {
    medicationOptions.forEach(opt => opt.classList.remove('active'));
    medicationOptions[0].classList.add('active'); // Default to first option (Wegovy)
  }
  ...
  medicationOptions.forEach(option => {
    option.addEventListener('click', function(e) {
      ...
      // Remove active from ALL options first
      ...
      console.log('Active medication:', this.dataset.medication);
    });
  });
  
  // Set initial data attribute
  if (medicationOptions.length > 0) {
    medicationOptions[0].setAttribute('data-active', 'true');
  }
